...
function notFoundStorageKey(routeId: string): string {
  return `smartroute:route:${routeId}:notfound`;
}
...
export function RouteView({ route, store, mapImageUrl, nodes, stopViews, previewMode = false }: RouteViewProps) {
  ...
  useEffect(() => {
    try {
      ...
      const rawNotFound = window.localStorage.getItem(notFoundStorageKey(route.id));
      if (rawNotFound) setNotFoundItemIds(new Set(JSON.parse(rawNotFound)));
    } catch {
  ...
  // Persisted too (mirrors checkedItemIds) so the Summary screen can record
  // which items were reported "not found" into the immutable trip history.
  useEffect(() => {
    if (!hydrated) return;
    window.localStorage.setItem(
      notFoundStorageKey(route.id),
      JSON.stringify(Array.from(notFoundItemIds)),
    );
  }, [notFoundItemIds, hydrated, route.id]);
