...
import { clearMyListCode, getMyListCode, setMyListCode } from "@/src/presentation/lib/my-list-storage";

interface HomeListWidgetProps {
  /**
   * For logged-in users the home page passes the account's active list
   * (server-resolved). When present it's authoritative and the localStorage
   * shareCode lookup is skipped.
   */
  serverMyList?: { list: ShoppingList; store: Store | null } | null;
  loggedIn?: boolean;
}

export function HomeListWidget({ serverMyList, loggedIn = false }: HomeListWidgetProps) {
  const router = useRouter();
  // Logged in: the server already resolved the active list, so state starts
  // populated and "checked". Guests start empty and resolve via localStorage.
  const [myList, setMyList] = useState<{ list: ShoppingList; store: Store | null } | null>(
    loggedIn ? (serverMyList ?? null) : null,
  );
  ...
  useEffect(() => {
    if (loggedIn) {
      // Mirror the active list's code into localStorage so the native widget /
      // share flow keep working. Pure external-system sync, no setState.
      if (serverMyList?.list.shareCode) setMyListCode(serverMyList.list.shareCode);
      return;
    }
